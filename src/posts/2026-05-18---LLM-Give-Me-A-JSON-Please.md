...
Let us thus take a deep dive into how do you go past "make no mistakes" and how do the inference engines do it reliable and fast.
...
while True:
  ...
  if is_json(answer):
    break
```
This works. The only positive thing I can think of it is, that you can treat the LLM as a complete blackbox, which might
be viable for some libraries (actually I believe this is what LangChain does). For the negatives, there are plenty:
- by being "unlucky" or employing smaler models, you can be looping for very long or forever, before reaching desired output
- youre wasting enormous number of tokens, by discarding whole messages, even though they might not be all wrong
...
First, LLMs generate outputs by token by token. Usually you don't have to generate the whole answer to see, that something is wrong.
...
Secondly, the answer tokens don't appear out of the blue. Given a text, LLMs produces a probability distribution on the next token.
...
With the (basic set of) regex features, you won't be able to for example guarantee, that any opened `{` will be also closed by corresponding `}`.

More fitting way for this use case is employing [JSON schemas](https://json-schema.org/).
...
Grammars are a precise descriptions on what strings can be generated - they are typically utilized in fields like programming language
...
INT    ::= "-"? [0-9]+
```
which is very simplified grammar for the JSON schema we discussed earlier. The STRING rule consumes any sequences without `"`,
...
and is underlying the constrained generation in llama.cpp, but also other engines.
...
```
This gets us a simplified expressions, which are easier to work with. If you feel a bit lost on what actually happened,
...
You can think about stack like an array, where from one end, we are allowed to append and pop elements.
...
For now, lets pretend it _just knows_:
```
stack: ["-", DIGITS]
string: -67
```
...
string: <empty>
```
Now that we arrived to an empty stack and empty string, we are ready to *accept*.
...
Again, lets imagine we are constrained with the rule INT:
```
INT ::= "-" DIGITS | DIGITS
DIGITS ::= [0-9] DIGITS | [0-9]
```
Before starting the inference, we will instantiate stack with the top rule - in this case `[INT]`.
```
stack: [INT]
```
Now its time to sample a token! We will look at all of the tokens one by one. Tokens can be various byte sequences in general:
```
vocab: {42, 13, 7b, haha, ...}
```
...
to ensure it wont be generated.
...
and in one go with `DIGITS`, where as the other one will have `[0-9] DIGITS`. Checking if a token goes through
is then about answering "Is there a stack which accepts?". Since all tokens are finite length, you dont have to care
about infinite recursion (having a stack that would always expand DIGITS branch).

And thats it! Conceputally, this is how llama.cpp works!
...
in time of the inference, it does not really scale well with bigger grammars, because you could be keeping
enormous amount of parralel stacks. In the end, you incur something like `O(vocab_size * active_stacks)` cost,
...
A next interesting step is going into the precomputation side of things - clearly,you dont want to do everything per step.
If you liked this, I would go for:
- reading this [incredible blogpost](https://guidance-ai.github.io/llguidance/llg-go-brrr) about how LLGuidance is built,
- looking at the [outlines paper](https://arxiv.org/pdf/2307.09702) to understand what you might cache before going generating,
...
print(answer)
```
If you value same things, come and [become a contributor](https://github.com/nobodywho-ooo/nobodywho)
